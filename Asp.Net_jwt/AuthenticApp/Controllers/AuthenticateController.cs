...
namespace AuthenticApp.Controllers
{
    ...
    public class AuthenticateController : ControllerBase
    {
        ...
        UserManager<IdentityUser> userManager, RoleManager<IdentityRole> roleManager, IConfiguration configuration)
        {
            ...
            _roleManager = roleManager ?? throw new ArgumentNullException(nameof(userManager));    
            _configuration = configuration ?? throw new ArgumentNullException(nameof(userManager));    
        }
        ...
        [HttpPost]
        [Route("register")]
        public async Task<IActionResult> Register([FromBody] ResgisterModel model)
        {
        ...
        public async Task<IActionResult> RegisterAdmin([FromBody] ResgisterModel model)
        {
            var userExists = await _userManager.FindByNameAsync(model.UserName);
            if (userExists != null)
                return StatusCode(StatusCodes.Status500InternalServerError, new Response { Status = "Error", Message = "User already exists!" });
        ...
        public async Task<IActionResult> RegisterEmployee([FromBody] ResgisterModel model)
        {
            var userExists = await _userManager.FindByNameAsync(model.UserName);
            if (userExists != null)
